/* ============================================================
   Reclaim Era — editable site data
   This file is the "publish" file for the admin dashboard
   (admin.html). To update the live site: replace this file
   with the one downloaded from the dashboard and re-upload.
   ============================================================ */
window.RE_DATA = {
  "phone": "+91 81520 20145",
  "stats": {
    "items": "22K+",
    "livelihoods": "13",
    "schools": "11"
  },
  "products": [
    {
      "id": "denim-tote",
      "name": "Denim Tote",
      "art": "tote",
      "price": "Price on request",
      "impact": "Funds 1 teacher for a week",
      "desc": "Reclaimed denim, reinforced handles. Stitched by a Batch-1 EmpowHer artisan.",
      "visible": true
    },
    {
      "id": "canvas-backpack",
      "name": "Canvas Backpack",
      "art": "backpack",
      "price": "Price on request",
      "impact": "Funds 10 notebooks",
      "desc": "Heavy canvas body with reclaimed lining. Fits a 15\" laptop.",
      "visible": true
    },
    {
      "id": "corporate-organizer",
      "name": "Corporate Organizer",
      "art": "organizer",
      "price": "Price on request",
      "impact": "Funds a school workshop",
      "desc": "Folio-style organizer with pen loop, a conference favourite.",
      "visible": true
    },
    {
      "id": "gift-bundle",
      "name": "Gift Bundle",
      "art": "bundle",
      "price": "Price on request",
      "impact": "Funds a student's supplies",
      "desc": "A curated set with a story card about the artisan who made it.",
      "visible": true
    }
  ]
};
